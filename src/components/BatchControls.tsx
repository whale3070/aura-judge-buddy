  ...
  return (
    <div className="mt-2.5 p-3 border border-border bg-background/80">
      <div className="flex gap-3 flex-wrap items-center text-sm">
        <span className="text-muted-foreground">目录过滤：</span>
        ...
        >
          <option value={1}>1 (稳)</option>
          <option value={2}>2</option>
          <option value={3}>3</option>
        </select>
        <span className="text-muted-foreground">间隔(ms)：</span>
        <input
          type="number"
          value={delayMs}
        ...
        <button onClick={onCollapseAll} className="bg-muted border border-border text-foreground/80 px-2.5 py-2 text-xs hover:bg-border transition-colors">全部折叠</button>
        <button onClick={onExpandAll} className="bg-muted border border-border text-foreground/80 px-2.5 py-2 text-xs hover:bg-border transition-colors">全部展开</button>
        <button onClick={onClear} className="bg-muted border border-border text-foreground/80 px-2.5 py-2 text-xs hover:bg-border transition-colors">清空输出</button>
      </div>
      ...
      <div className="mt-2 text-xs text-muted-foreground">
        {progress.total === 0 ? "批量任务未开始。" : `进度：${progress.done}/${progress.total} (${pct}%)`}
      </div>
    </div>
  );
}
